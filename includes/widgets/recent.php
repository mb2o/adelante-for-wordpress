 ...
 */
class adelante_widget_recent extends WP_Widget 
{
	function adelante_widget_recent() 
    {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => __( "Displays the recent posts on your site", 'adelante') );
		$this->WP_Widget('recent_posts', THEME_SLUG.' - '.__('Recent Posts', 'adelante'), $widget_ops);
	...
	function widget($args, $instance) 
    {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Posts', 'adelante') : $instance['title'], $instance, $this->id_base);
...
<?php if ( ! $disable_thumbnail ) :?>
            <figure class="widget-recent-img">
                <a class="thumbnail" href="<?php echo get_permalink() ?>" title="<?php the_title();?>">
                    <?php if (has_post_thumbnail() ): ?>                    
                        ...
                        ?>
                        <img src="<?php echo adelante_image_resize(65, 65, "http://". $_SERVER['SERVER_NAME']. $image[0]); ?>" alt="<?php the_title();?>" title="<?php the_title();?>" />        
                    <?php else:?>
